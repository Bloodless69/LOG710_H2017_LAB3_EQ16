...
void testAlloueAllMemory() {
	initMemory(500);
	struct bloc *bloc = allocateMemory(worstFitStrategy, 500);
...
void testAlloueMemoryWithLeftOver() {
	initMemory(500);
	struct bloc *bloc = allocateMemory(worstFitStrategy, 100);
...
void testAlloueMemoryGotNext() {
	...
	initMemory(500);
	struct bloc *bloc = allocateMemory(worstFitStrategy, 100);
	struct bloc *nextBloc = allocateMemory(worstFitStrategy, 100);
	freeMemory(bloc);
	struct bloc *newNextBloc = allocateMemory(worstFitStrategy, 50);
...
void testNotEnoughMemory() {
	initMemory(500);
	assert(allocateMemory(worstFitStrategy, 600) == NULL);
}
/////////////////////////////////////////////////////
