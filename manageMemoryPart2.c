...
struct node *lastAllocatedNode = NULL; // used for firstFitLoopingStrategy
...
void initMemory(int memSize) {
	...
	root->previous = NULL;
	root->next = NULL;
	lastAllocatedNode = root;
}

node* firstFitStrategy(int blocSize) {
...
node* bestFitStrategy(int blocSize) {
	struct node* currentNode = root;
	int currentFit;
    ...
    while(currentNode != NULL) {
    	if(currentNode->value->data == NULL && currentNode->value->size >= blocSize) {
    		if(bestFitNode == NULL) {
    			currentFit = currentNode->value->size - blocSize;
    			bestFitNode = currentNode;
    		} else {
    			if (currentFit >= currentNode->value->size - blocSize) {
    				currentFit = currentNode->value->size - blocSize;
    				bestFitNode = currentNode;
    			}
    		}
...
node* worstFitStrategy(int blocSize) {
	struct node* currentNode = root;
	int currentFit;
	struct node* worstFitNode = NULL;
    while(currentNode != NULL) {
    	if(currentNode->value->data == NULL && currentNode->value->size >= blocSize) {
    		if(worstFitNode == NULL) {
    			currentFit = currentNode->value->size - blocSize;
    			worstFitNode = currentNode;
    		} else {
    			if (currentFit <= currentNode->value->size - blocSize) {
    				currentFit = currentNode->value->size - blocSize;
    				worstFitNode = currentNode;
...
node* firstFitWithLoopingStrategy(int blocSize) {
	struct node* currentNode = lastAllocatedNode;
    while(true) {
    	if(currentNode->value->data == NULL && currentNode->value->size >= blocSize) {
    		return currentNode;
    	} else {
            ...
            if(lastAllocatedNode == currentNode) {
    			return NULL;
    		}
        }
    }
...
// allouemem()
bloc* allocateMemory(node* (*getNode)(int), int blocSize) {
    struct node *chosenNode = getNode(blocSize);
    if(chosenNode == NULL)
    	return NULL;
    struct bloc *currentBloc = chosenNode->value;
    // Creating our bloc of memory to allocate
    struct bloc *allocationBloc = allocationBloc = malloc(sizeof(bloc));
    allocationBloc->size = blocSize;
    allocationBloc->offset = chosenNode->value->offset;
    ...
    if(currentBloc->size > blocSize) {
        ...
        emptyNode->previous = chosenNode;
        if(chosenNode->next != NULL) {
            emptyNode->next = chosenNode->next;
            node *rightNode = chosenNode->next;
        ...
        chosenNode->next = emptyNode;
        chosenNode->value = allocationBloc;
    } else {
        chosenNode->value = allocationBloc;
    }
